...
dir="/data/activate_data/ranxiaojuan/ranxiaojuan_GBM_vs_PBMC_analysis/20230919_revision_code/01_SoupX/"
samples <- c(paste0("GBM",c(1:9)),paste0("PBMC",c(1:9)))
sceList <- lapply(samples, function(x){
    #x <- samples[1]
    data <- readRDS(paste0(dir,x,"_out/",x,"_singlet.rds"))
})
...
p <- PCAPlot(object = merged,raster=FALSE)
ggsave(p,file="merged_cellcylce.pdf",width=5,height=5)

#harmony remove batch effect
p1 <- DimPlot(object = merged, reduction = "pca", group.by = "orig.ident",raster=FALSE)
...
p1 <- DimPlot(object = merged, reduction = "harmony", group.by = "orig.ident",raster=FALSE)
...
p <- ElbowPlot(merged,reduction="harmony")
...
merged <- SetIdent(merged, value="SCT_snn_res.0.6")
p1 <- DimPlot(merged, reduction = "umap", group.by = "orig.ident",raster=FALSE)
p2 <- DimPlot(object = merged, reduction = 'umap', label=TRUE, label.size=5,raster=FALSE)
ggsave(plot_grid(p1,p2),file="merged_umap.pdf",width=14,height=6)
p <- DimPlot(object = merged, reduction = 'umap', label=TRUE, label.size=5,split.by="orig.ident",ncol=10,raster=FALSE)
ggsave(p, file="merged_umap_split.pdf",width=40,height=5)
p1 <- DimPlot(merged, reduction = "tsne", group.by = "orig.ident",raster=FALSE)
p2 <- DimPlot(object = merged, reduction = 'tsne', label=TRUE, label.size=5,raster=FALSE)
ggsave(plot_grid(p1,p2),file="merged_tsne.pdf",width=14,height=6)
p <- DimPlot(object = merged, reduction = 'tsne', label=TRUE, label.size=5,split.by="orig.ident",ncol=10,raster=FALSE)
...
p <- DoHeatmap(merged,features = genes,size=3,draw.lines=TRUE,combin=TRUE,raster=FALSE)+scale_fill_gradientn(colors = c("blue", "white", "red"),na.value="white")+theme(text = element_text(size=12),axis.text = element_text(color="black"))
ggsave(p, file="marker_heatmap.pdf",width=30,height=12)


sub <- subset(merged,idents=c(25:32))
p <- DoHeatmap(sub,features = genes,size=3,draw.lines=TRUE,combin=TRUE,raster=FALSE)+scale_fill_gradientn(colors = c("blue", "white", "red"),na.value="white")+theme(text = element_text(size=12),axis.text = element_text(color="black"))
ggsave(p, file="sub_marker_heatmap.pdf",width=30,height=12)


sub <- subset(merged,idents=c(18:24))
p <- DoHeatmap(sub,features = genes,size=3,draw.lines=TRUE,combin=TRUE,raster=FALSE)+scale_fill_gradientn(colors = c("blue", "white", "red"),na.value="white")+theme(text = element_text(size=12),axis.text = element_text(color="black"))
ggsave(p, file="sub_marker_heatmap2.pdf",width=30,height=12)
...
merged@meta.data$anno[which(merged@meta.data$SCT_snn_res.0.6 %in% c(10))] = "NK"
merged@meta.data$anno[which(merged@meta.data$SCT_snn_res.0.6 %in% c(1,2,5,23,26))] = "T"
merged@meta.data$anno[which(merged@meta.data$SCT_snn_res.0.6 %in% c(24))] = "Tumor"
merged@meta.data$anno[which(merged@meta.data$SCT_snn_res.0.6 %in% c(15,16))] = "B"
merged@meta.data$anno[which(merged@meta.data$SCT_snn_res.0.6 == 12)] = "DC"
merged@meta.data$anno[which(merged@meta.data$SCT_snn_res.0.6 %in% c(3,11,18))] = "Monocyte"
merged@meta.data$anno[which(merged@meta.data$SCT_snn_res.0.6 %in% c(0,4,6,8,9,14,19))] = "Macrophage"
merged@meta.data$anno[which(merged@meta.data$SCT_snn_res.0.6 %in% c(28))] = "Mast"
merged@meta.data$anno[which(merged@meta.data$SCT_snn_res.0.6 %in% c(7,13,17,20,21,22,25,27,29,30,31,32))] = "Doublets"
...
data1 <- SetIdent(data1, value="SCT_snn_res.0.6")
p1 <- DimPlot(data1, reduction = "umap", group.by = "orig.ident", raster=FALSE)
p2 <- DimPlot(object = data1, reduction = 'umap', label=TRUE, label.size=5, raster=FALSE)
ggsave(plot_grid(p1,p2),file="data1_umap.pdf",width=14,height=6)
p <- DimPlot(object = data1, reduction = 'umap', label=TRUE, label.size=5,split.by="orig.ident",ncol=10, raster=FALSE)
ggsave(p, file="data1_umap_split.pdf",width=40,height=5)
p1 <- DimPlot(data1, reduction = "tsne", group.by = "orig.ident", raster=FALSE)
p2 <- DimPlot(object = data1, reduction = 'tsne', label=TRUE, label.size=5, raster=FALSE)
ggsave(plot_grid(p1,p2),file="data1_tsne.pdf",width=14,height=6)
p <- DimPlot(object = data1, reduction = 'tsne', label=TRUE, label.size=5,split.by="orig.ident",ncol=10, raster=FALSE)
ggsave(p, file="data1_tsne_split.pdf",width=40,height=5)


genes <- c("CD3D","CD3E","CD3G","CD8A","CD8B","CD4","CD40LG","NCAM1","NCR1","KLRF1","CD19","CD79A","CD79B","MS4A1","CD14","LYZ","FCN1", "S100A8","S100A9","MNDA","CD68","ITGAM", "CD163","AIF1", "CST3","C1QA","CD1C","FCER1A","CD1E","CLEC10A","MS4A2","HDC","CTSG","SOX2","SPARC","GFAP","IGFBP7","CLU","FAP","ACTA2","PDGFRA","PDGFRB")
p <- DoHeatmap(data1,features = genes,size=3,draw.lines=TRUE,combin=TRUE,raster=FALSE)+scale_fill_gradientn(colors = c("blue", "white", "red"),na.value="white")+theme(text = element_text(size=12),axis.text = element_text(color="black"))
ggsave(p, file="marker_heatmap.pdf",width=30,height=12)
...
#remove tumor
data1 <- SetIdent(data1, value="anno")
data <- subset(data1,idents=c("Monocyte","Macrophage","DC","Mast","B","NK","T"))
data <- SetIdent(data,value="anno")
saveRDS(data,file="data_anno.rds")
...
pdf("Percent_stackbar_PBMC_vs_GBM_v2.pdf",width=10,height=4)
ggplot(df,aes(fill=factor(celltype,levels=c("Monocyte","Macrophage/Microglia","Dendritic cell","Mast","B","NK","T")),y=Fraction,x=sample))+geom_bar(position="stack", stat="identity")+scale_fill_manual(values=c("#E64B35B2","#4DBBD5B2","#00A087B2","#3C5488B2","#F39B7FB2","#8491B4B2","#91D1C2B2"))+theme_classic()+theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_rect(colour = "black", size=0.5), axis.text = element_text(size=12,color="black"),axis.text.x = element_text(size = 12,angle = 90, vjust = 0.5, hjust=1),axis.text.y = element_text(size = 12),legend.text = element_text(size = 12))
dev.off()
...
pdf("Percent_bar_PBMC_vs_GBM_v3.pdf",width=10,height=2.5)
...
pdf("Correction_marker.pdf",width=3,height=30)
StackedVlnPlot(obj = data, features = genes)
dev.off()


data_nocorr <- readRDS("/data/activate_data/ranxiaojuan/ranxiaojuan_GBM_vs_PBMC_analysis/9sample_v2/Glioma_vs_PBMC_anno.rds")

pdf("noCorrection_marker.pdf",width=3,height=30)
StackedVlnPlot(obj = data_nocorr, features = genes)
dev.off()
